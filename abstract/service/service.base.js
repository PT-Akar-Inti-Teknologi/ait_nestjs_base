...
class BaseService {
     ...
     * @param queryBuilder - query builder
     */
    baseFilterAllQuery(mainPagingDTO, filterQuery, queryBuilder) {
        for (let i = 0; i < filterQuery.length; i++) {
            if (Array.isArray(filterQuery[i])) {
                queryBuilder.andWhere(new typeorm_1.Brackets((qb) => {
                    this.baseFilterAllQuery(mainPagingDTO, filterQuery[i], qb);
                }));
            }
            else {
                this.baseFilterQuery(mainPagingDTO, filterQuery[i], queryBuilder);
            }
        }
    }
    /**
